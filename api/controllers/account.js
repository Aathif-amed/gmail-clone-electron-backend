...
export async function login(request, response, next) {
  try {
    ...
    const foundAccount = await Account.findOne({ email: request.body.email });
    if (!foundAccount) return response.status(401).json({ message: 'Invalid Email/Password' });

    const isPasswordOk = await bcrypt.compare(request.body.password, foundAccount.password);
    if (!isPasswordOk) return response.status(401).json({ message: 'Invalid Email/Password' });
